...
treatment_city <- "Houston, TX"
control_cities <- c(
  "Austin, TX", "Beaumont, TX", "Brownsville, TX", "College Station, TX", "Corpus Christi, TX",
  "Lewisville, TX", "El Paso, TX-NM", "Laredo, TX", "Lubbock, TX", "Odessa, TX", "San Antonio, TX",
  "Waco, TX"
)
...
filtered_data <- ntd_data %>%
  ...
  filter(year < 2012) %>%
  mutate(
    group = ifelse(Urbanized.Area == treatment_city, "Treatment City (Houston, TX)", "Control Cities"),
    period = case_when(
...
ggplot(average_data, aes(x = Urbanized.Area, y = average_people, fill = transportation)) +
  ...
  theme_minimal() +
  labs(
    title = "Average Number of People Using Different Transportation Modes by City (2000-2007)",
    x = "City",
