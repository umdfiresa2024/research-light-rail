...
Download shapefiles from this website <https://www.census.gov/cgi-bin/geo/shapefiles/index.php>

```{r}
block_shapes<-vect("Phoenix Shapefiles/tl_2010_04013_tabblock00/tl_2010_04013_tabblock00.shp")
plot(buff)
lines(block_shapes, col="grey")

blockdf<-as.data.frame(block_shapes)
```

```{r}
racename2<-racename %>%
  mutate(BLKIDFP00=paste0(substr(GEOID,1,10), "0", substr(GEOID,11,15)))

blockrace<-merge(block_shapes, racename, by.x="BLKIDFP00", by.y="GEOID")

blockracedf<-as.data.frame(blockrace)

#check digits
nchar(racename2$BLKIDFP00)[1]
nchar(blockdf$BLKIDFP00)[1]
```
