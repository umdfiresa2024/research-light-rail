...
```{r}
blockrace<-merge(shape, racename, by.x="GEOID10", by.y="GEOID")
...
intdf<-as.data.frame(int) %>%
  ...
  mutate(total=total*frac_area, white=white*frac_area, black=black*frac_area, native=native*frac_area, 
         asian=asian*frac_area, hisp=hisp*frac_area) %>%
  summarize(total=sum(total), white=sum(white), black=sum(black), native=sum(native), asian=sum(asian), hisp=sum(hisp))
```
