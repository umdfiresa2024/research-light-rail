...
bus_types<-c("MB", "MBp", "RB", "RBp", "CB","CBp", "TB")
oth_types<-c("CC", "FB","FBp", "IP","IPp", "TR", "DR","DRp")
rail_types<-c("CR","CRp", "HR", "YR", "MG", "SR")
car_types<-c("DT", "VP","VPp", "JT")
...
```{r}
df <- read.csv("NTD/2000_Operating_Stats.csv")

df2<-df |>
  rename(ID=Org) |>
  rename(Unlinked.Passenger.Trips=`Annual.Unlinked.Passenger.Trips`) |>
  select(ID, Mode, Unlinked.Passenger.Trips) |>
  mutate(UPT=str_remove_all(Unlinked.Passenger.Trips, ",")) |>
  mutate(UPT=as.numeric(UPT)) |>
  mutate(mode_group=ifelse(Mode %in% bus_types, "bus", NA)) |>
...
```{r}
df <- read.csv("NTD/2001_Operating_Stats.csv")
...
```{r}
df <- read.csv("NTD/2002_Operating_Stats.csv")

df2<-df |>
  rename(Unlinked.Passenger.Trips=Unlinked.Passenger.Trips) |>
  select(ID, Mode, Unlinked.Passenger.Trips) |>
  mutate(UPT=str_remove_all(Unlinked.Passenger.Trips, ",")) |>
  mutate(UPT=as.numeric(UPT)) |>
  mutate(mode_group=ifelse(Mode %in% bus_types, "bus", NA)) |>
...
```{r}
df <- read.csv("NTD/2003_Operating_Stats.csv")
...
all<-rbind(df_y00, df_y01, df_y02, df_y03, df_y04, df_y05, df_y06, df_y072, 
           df_y082, df_y092, df_y102, df_y112, df_y122) |>
  group_by(ID, year) |>
...
write.csv(all, "ntd_00to12.csv", row.names=F)
```
